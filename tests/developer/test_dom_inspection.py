...
async def test_inspect_dom_basic(mock_context):
    ...
    mock_second.evaluate = AsyncMock(
        side_effect=[
            "button",  # tag name
    ...
    with patch(
        ...
        with patch(
            ...
            result = await inspect_dom(
                mock_context, url="http://localhost:8000", selector="button"
            )
...
async def test_inspect_dom_no_elements(mock_context):
    ...
    with patch(
        ...
        with patch(
            ...
            result = await inspect_dom(
                mock_context, url="http://localhost:8000", selector=".nonexistent"
            )
...
async def test_inspect_dom_limits_to_50_elements(mock_context):
    ...
    mock_nth_element.evaluate = AsyncMock(
        side_effect=lambda script: "div" if "tagName" in script else {}
    )
    ...
    with patch(
        ...
        with patch(
            ...
            result = await inspect_dom(
                mock_context, url="http://localhost:8000", selector="div"
            )
...
async def test_inspect_dom_with_element_id(mock_context):
    ...
    with patch(
        ...
        with patch(
            ...
            result = await inspect_dom(
                mock_context, url="http://localhost:8000", element_id="my-element"
            )
...
async def test_inspect_dom_requires_selector_or_id(mock_context):
    ...
    with patch(
        ...
        result = await inspect_dom(mock_context, url="http://localhost:8000")
...
async def test_inspect_dom_handles_element_errors(mock_context):
    ...
    mock_first = AsyncMock()
    mock_first.evaluate = AsyncMock(side_effect=["div", {"class": "good"}])
