...
class TestDestroyCommand:
    ...
    def test_get_app_name_uses_correct_workspace(self, mock_get_silica_dir, mock_find_git_root, multi_workspace_config):
        ...
        assert app_name_default == "second-silica", "Should use default workspace 'second'"
        
        # Test getting app name for specific workspace (agent)
        app_name_agent = get_app_name(git_root, workspace_name="agent")
        assert app_name_agent == "agent-silica", "Should use specified workspace 'agent'"
        
        # Test getting app name for specific workspace (second)
        app_name_second = get_app_name(git_root, workspace_name="second")
        assert app_name_second == "second-silica", "Should use specified workspace 'second'"
    ...
    def test_destroy_command_uses_specified_workspace(
        ...
        with patch('silica.cli.commands.destroy.get_app_name') as mock_get_app_name:
            with patch('silica.cli.commands.destroy.get_piku_connection') as mock_get_piku_connection:
                ...
                result = runner.invoke(destroy, ['--workspace', 'agent', '--force'])
                ...
                mock_get_piku_connection.assert_called_with(git_root, workspace_name='agent')
    ...
    def test_destroy_command_default_workspace_consistency(
        ...
        with patch('silica.cli.commands.destroy.get_app_name') as mock_get_app_name:
            with patch('silica.cli.commands.destroy.get_piku_connection') as mock_get_piku_connection:
                ...
                result = runner.invoke(destroy, ['--force'])
