...
class CompactionTester:
    ...
    def run_compaction(
        ...
        print(f"  Threshold ({int(self.compacter.threshold_ratio * 100)}%): {threshold:,}")
        print(f"  Utilization: {(token_count / context_window * 100):.1f}%")
        
        # Perform compaction
        print("\n⏳ Generating compaction summary...")
        compacted_messages, summary = self.compacter.compact_conversation(context, model)
    ...
    def save_compacted_session(
        ...
        new_session_data = {
            ...
            "metadata": {
                "created_at": original_session_data.get("metadata", {}).get("created_at"),
                "last_updated": original_session_data.get("metadata", {}).get("last_updated"),
    ...
    def test_session(
        ...
        if not dry_run:
            print("\n" + "=" * 70)
            response = input("Proceed with compaction? This will call the Anthropic API. (y/N): ").strip().lower()
            if response != 'y':
        ...
        model = session_data.get("model_spec", {}).get("title", "claude-3-5-sonnet-latest")
        ...
        # Save if requested
        if save_output:
            self.save_compacted_session(
                compacted_messages, session_data, summary
            )
...
Examples:
  ...
  python scripts/test_compaction.py --session-id abc-123-def --verbose
    ...
    parser.add_argument(
        "--history-file",
        type=Path,
        help="Path to specific root.json file to test"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--no-save",
        action="store_true",
        help="Don't save the compacted session"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
